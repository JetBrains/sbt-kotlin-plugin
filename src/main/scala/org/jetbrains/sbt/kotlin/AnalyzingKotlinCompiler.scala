...
class AnalyzingKotlinCompiler(
  ...
  def compile(
    ...
    val allSourceFiles = allSources.map(vf => converter.toPath(vf))

    val kotlinSources = allSourceFiles.filter { f =>
      val name = f.getFileName.toString
      name.endsWith(".kt") || name.endsWith(".kts")
    }
    val javaSources = allSourceFiles.filter(_.getFileName.toString.endsWith(".java"))
    ...
    if (kotlinSources.nonEmpty || javaSources.nonEmpty) {
      ...
      val message =
        if (kotlinSources.nonEmpty) {
          ...
          if (javaSources.nonEmpty) {
            ...
            s"compiling $ktCount Kotlin ${pluralizeSource(ktCount)} and $javaCount Java ${pluralizeSource(javaCount)} to ${out.toAbsolutePath.normalize()} ..."
          } else {
            s"compiling $ktCount Kotlin ${pluralizeSource(ktCount)} to ${out.toAbsolutePath.normalize()} ..."
          }
        } else {
          val javaCount = javaSources.size
          s"compiling $javaCount Java ${pluralizeSource(javaCount)} to ${out.toAbsolutePath.normalize()} ..."
        }
      log.info(message)
      ...
      if (kotlinSources.nonEmpty) {
        ...
        timed(kotlinCompilationPhase, log) {
          val compilerClasspathAsNioPaths = PluginCompat.toNioPaths(compilerClasspath, converter)
          val stub = KotlinStub(log, KotlinCompile.memoizedKotlinReflection(compilerClasspathAsNioPaths))
          ...
          args.friendPaths = Array(out.toAbsolutePath.normalize().toString)
          args.freeArgs = (kotlinSources ++ javaSources).map(_.toAbsolutePath.normalize().toString).asJava
          val fcpjars = classpath.filter(Files.exists(_))
          ...
          val cp = out.toAbsolutePath.normalize().toString + File.pathSeparator + cpjars.mkString(File.pathSeparator)
          val pcp = pluginjars.map(_.toAbsolutePath.normalize().toString).toArray
          ...
          args.destination = out.toAbsolutePath.normalize().toString
      ...
      if (javaSources.nonEmpty) {
        ...
        timed(javaCompilationPhase, log) {
          val absoluteClasspath = converter.toVirtualFile(out) +: classpath.map(converter.toVirtualFile)
          ...
          val javaSrcs = javaSources.map(converter.toVirtualFile).toArray
      ...
      val loader = ClasspathUtil.toLoader(
        Seq(out) ++ classpath ++ searchClasspath.map(converter.toPath)
      )
  ...
  private def grepjar(jarfile: Path)(pred: JarEntry => Boolean): Boolean =
    Files.isRegularFile(jarfile) && Using.jarFile(false)(jarfile.toFile) { in =>
